...
from triplet.models import Decoder
...
for i in range(1, num_class + 1):
    ...
    if n > 0:
        try:
            z_samp, class_labels = generate_balanced_inputs(yclass, n, i, args['n_z'], num_classes=num_class)
        ...
        if not (class_labels_t >= 0).all() or not (class_labels_t < 8).all():
            print(f"Invalid class labels for class {i}: {class_labels_t}")
            continue
        ...
        for j in range(0, len(z_samp), args['batch_size']):
            ...
            with torch.no_grad():
                ximg = decoder(batch_z, batch_labels)
